...
Value ScriptHost::builtinSelfPos(int axis)
{
    ...
    return Value::makeNumber(axis == 0 ? p.x : axis == 1 ? p.y : p.z);
}
Value ScriptHost::builtinSelfRot(int axis)
{
    ...
    glm::vec3 p, r, s;
    selfObj->getTransform(p, r, s);
    return Value::makeNumber(axis == 0 ? r.x : axis == 1 ? r.y : r.z);
}
Value ScriptHost::builtinSelfScale(int axis)
{
    ...
    glm::vec3 p, r, s;
    selfObj->getTransform(p, r, s);
    return Value::makeNumber(axis == 0 ? s.x : axis == 1 ? s.y : s.z);
}
...
Value ScriptHost::builtinSpawn(const std::vector<Value> &args, const std::string &kind)
{
    ...
    else if (kind == "cow")
    {
        ...
        if (!mesh)
            mesh = am.loadStaticMeshFromArrays("cow", cow_mesh_vertices, cow_mesh_vertex_count,
                                               cow_mesh_indices, cow_mesh_index_count, 3);
        if (mesh)
        {
...
namespace
{
    Value transformGet(Object *o, const std::string &prop)
    {
        ...
        if (prop == "rx") return Value::makeNumber(r.x);
        if (prop == "ry") return Value::makeNumber(r.y);
        if (prop == "rz") return Value::makeNumber(r.z);
        if (prop == "sx") return Value::makeNumber(s.x);
        if (prop == "sy") return Value::makeNumber(s.y);
        if (prop == "sz") return Value::makeNumber(s.z);
    ...
    Value rigidbodyGet(Object *o, const std::string &prop)
    {
        ...
        if (prop == "mass") return Value::makeNumber(o->getMass());
